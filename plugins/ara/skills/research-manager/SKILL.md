...
## The Four-Stage Pipeline & State Referee

**CRITICAL PROTOCOL:** You MUST execute this pipeline using the strict State Referee script. 
1. Initialize the referee: `python ~/.gemini/config/plugins/ara/skills/research-manager/scripts/manager_referee.py init`
2. Follow the prompt on stdout exactly.
3. Once a stage is complete, advance: `python .../manager_referee.py advance --to [NEXT_PHASE]`
...
**CRITICAL:** You must wrap these steps in the State Referee CLI.

```
1. Initialize Referee: `python scripts/manager_referee.py init`.
2. Read existing ara/ files (current state, next IDs).
3. Stage 1 (HARVEST) — harvest this turn's candidate events.
4. Advance Referee: `python scripts/manager_referee.py advance --to ROUTE`.
5. Stage 2 (ROUTE) — classify/route each (per event-taxonomy.md): journey facts direct to trace/; interpretive events staged to staging/observations.yaml.
6. Advance Referee: `python scripts/manager_referee.py advance --to TRACK_MATURITY`.
7. Stage 3 (TRACK_MATURITY) — crystallize staged observations whose closure signal fired; flag contradictions; mark 3+-day-idle observations stale.
8. Advance Referee: `python scripts/manager_referee.py advance --to RECONCILE`.
9. Stage 4 (RECONCILE) — for each crystallized logic/ entry, apply status/content/structural edits when a signal fires; run the cross-ref consistency pass; record before/after in the session record; log near-misses.
10. Append turn events to today's session record; update session_index.yaml; append a line to pm_reasoning_log.yaml.
11. Print one-line summary, e.g.:

...
| Situation | Action |
|---|---|
| `paperorchestra/inputs/idea.md` **and** `paperorchestra/inputs/experimental_log.md` both exist and are non-empty | **Skip this skill entirely.** Proceed directly to `paper-orchestra`. |
...
[.claude/]  [.cursor/]  [.antigravity/]  [.openclaw/]
      ...
      └────────────┴──────────────┴───────────────┘
                  ...
                  (format_po_inputs.py)
                          │
             ┌────────────┴────────────┐
      paperorchestra/inputs/         paperorchestra/ara/
...
The output drops directly into `paperorchestra/inputs/` so the user can immediately
...
| `--paperorchestra` | no | `./paperorchestra` | PaperOrchestra workspace root |
...
python skills/agent-research-aggregator/scripts/discover_logs.py \
    ...
    --depth <depth> \
    --since <since> \
    --out paperorchestra/ara/discovered_logs.json
```
...
5. **Append** to `paperorchestra/ara/raw_experiments.json`.

After all batches:

```bash
python skills/agent-research-aggregator/scripts/extract_experiments.py \
    --discovered paperorchestra/ara/discovered_logs.json \
    --out paperorchestra/ara/raw_experiments.json \
    --validate-only
```
...
```
<raw_experiments>
{contents of paperorchestra/ara/raw_experiments.json}
</raw_experiments>
```
...
Save to `paperorchestra/ara/synthesis.json`.
...
python skills/agent-research-aggregator/scripts/format_po_inputs.py \
    --synthesis paperorchestra/ara/synthesis.json \
    --out paperorchestra/inputs/
```

This generates two files:

### `paperorchestra/inputs/idea.md` (Sparse variant)
...
### `paperorchestra/inputs/experimental_log.md`
...
1. Read `paperorchestra/inputs/idea.md` aloud and ask: "Does this accurately capture
   your research question and method?"
2. Read the table headers from `paperorchestra/inputs/experimental_log.md` and ask:
...
python skills/agent-research-aggregator/scripts/format_po_inputs.py \
    --synthesis paperorchestra/ara/synthesis.json \
    --out paperorchestra/inputs/ \
    --report paperorchestra/ara/aggregation_report.md
```
...
| File | Status | Action |
|---|---|---|
| `paperorchestra/inputs/idea.md` | ✓ generated | user review recommended |
| `paperorchestra/inputs/experimental_log.md` | ✓ generated | user review recommended |
| `paperorchestra/inputs/template.tex` | **MISSING** | ask user to provide their conference LaTeX template |
| `paperorchestra/inputs/conference_guidelines.md` | **MISSING** | ask user to provide (page limit, deadline, formatting rules) |
...
python skills/agent-research-aggregator/scripts/discover_logs.py \
    --search-roots . ~ --out paperorchestra/ara/discovered_logs.json
...
python skills/agent-research-aggregator/scripts/extract_experiments.py \
    --discovered paperorchestra/ara/discovered_logs.json \
    --out paperorchestra/ara/raw_experiments.json --validate-only

...
grep -in -E '\blimitation' paperorchestra/refinement/iter<N>/paper.tex \
    | grep -v '^\s*%'
```
...
python skills/section-writing-agent/scripts/orphan_cite_gate.py \
    paperorchestra/refinement/iter<N>/paper.tex \
...
# 2. compile
cd paperorchestra/refinement/iter<N>/ && latexmk -pdf -interaction=nonstopmode paper.tex

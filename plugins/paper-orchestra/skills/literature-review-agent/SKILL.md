...
- `paperorchestra/outline.json` — specifically `intro_related_work_plan` with the
  Introduction search directions and the 2-4 Related Work methodology
  clusters
- `paperorchestra/inputs/conference_guidelines.md` — used to derive `cutoff_date`
- `paperorchestra/inputs/idea.md`, `paperorchestra/inputs/experimental_log.md` — for
  framing the Intro and grounding the Related Work positioning

## Outputs

- `paperorchestra/citation_pool.json` — verified Semantic Scholar metadata for
  every paper that survived verification
- `paperorchestra/refs.bib` — BibTeX file generated from the verified pool
- `paperorchestra/drafts/intro_relwork.tex` — drafted Introduction and Related
...
python skills/literature-review-agent/scripts/pre_dedup_candidates.py \
    --in paperorchestra/raw_candidates.json \
    --out paperorchestra/deduped_candidates.json
# Prints: "150 candidates → 97 unique (53 duplicates removed)"
```

Use `paperorchestra/deduped_candidates.json` as input to Phase 2.
...
python skills/literature-review-agent/scripts/s2_cache.py \
    --cache paperorchestra/cache/s2_cache.json \
...
python skills/literature-review-agent/scripts/dedupe_by_id.py \
    --in raw_pool.json \
    --out paperorchestra/citation_pool.json
```
...
python skills/literature-review-agent/scripts/validate_pool.py \
    --pool paperorchestra/citation_pool.json --fix
...
python skills/literature-review-agent/scripts/cross_verify.py \
    --pool paperorchestra/citation_pool.json --inplace
# Annotates each paper with a `cross_verification` field and writes
# paperorchestra/cross_verification_report.json.
...
python skills/literature-review-agent/scripts/bibtex_format.py \
    --pool paperorchestra/citation_pool.json \
    --out paperorchestra/refs.bib
```
...
python skills/literature-review-agent/scripts/sync_keys.py \
    --pool paperorchestra/citation_pool.json \
    --tex  paperorchestra/drafts/intro_relwork.tex \
    --inplace
...
`paperorchestra/drafts/intro_relwork.tex`.

### 5b. Append §2 to research_brief.md

After `intro_relwork.tex` is drafted and before the citation coverage check,
append §2 to `paperorchestra/research_brief.md` (see `skills/shared/research_brief_template.md`).
...
1. If the user has placed a pre-built `paperorchestra/inputs/refs.bib` in the
   workspace, load it directly into `paperorchestra/refs.bib` and skip Phase 1
   and Phase 2.
2. Otherwise, emit `paperorchestra/drafts/intro_relwork.tex` containing the

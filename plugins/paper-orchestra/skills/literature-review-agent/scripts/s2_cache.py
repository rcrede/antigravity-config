...
Solution: a flat JSON cache at paperorchestra/cache/s2_cache.json. On a cache
...
Usage:
  ...
  CHECK mode — exits 0 + prints JSON if cached, else exits 1:
    python s2_cache.py --cache paperorchestra/cache/s2_cache.json \\
  ...
  STATS mode — print cache size and hit rate summary:
    python s2_cache.py --cache paperorchestra/cache/s2_cache.json --stats
"""
import argparse
import json
import os

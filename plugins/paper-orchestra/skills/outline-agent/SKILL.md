...
`paperorchestra/outline.json` with three top-level keys:
...
3. **Read the four input sources**:
   - `ara/logic/claims.md` (the core scientific claims and logic)
   - The files in `ara/evidence/` (raw data, tables, figures)
   - `paperorchestra/inputs/template.typ` (the Typst template)
...
6. **Save to** `paperorchestra/outline.json`.
7. **Validate**:
   ```bash
   python skills/outline-agent/scripts/validate_outline.py paperorchestra/outline.json
   ```
...
8. **Append §1 to research_brief.md** (see `skills/shared/research_brief_template.md`):
   ...
   `paperorchestra/research_brief.md` (create the file if absent). Template:
...
- `data_source` MUST be exactly one of `"ara_logic"`, `"ara_evidence"`,
  or `"both"`.
...
- **Mandatory citations**: every dataset, optimizer, metric, and
  foundational architecture/model mentioned in `ara/logic/claims.md` or
  `ara/evidence/` MUST have a citation hint, no matter how ubiquitous
...
Exactly one file: `paperorchestra/outline.json`. No prose, no code blocks, no

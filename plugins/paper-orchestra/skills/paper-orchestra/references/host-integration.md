...
mode: it uses only any user-provided BibTeX in `paperorchestra/inputs/` and emits
...
3. Scaffold a workspace and drop your inputs:
   ```
   ! python ~/paper-orchestra/skills/paper-orchestra/scripts/init_workspace.py --out paperorchestra/
   ```
4. Ask Claude:
   > Run the paper-orchestra pipeline on `./paperorchestra`.
...
1. Aider is text-only and lacks built-in web search or vision. Use the
   degraded mode: provide a pre-built BibTeX in `paperorchestra/inputs/refs.bib`
...
If you encounter this in a compiled PDF, edit `paperorchestra/final/paper.tex`
...
```python
import json
with open('paperorchestra/citation_pool.json') as f:
    pool = json.load(f)
key_map = {p['key']: p['bibtex_key'] for p in pool['papers']}
with open('paperorchestra/drafts/intro_relwork.tex') as f:
    content = f.read()
for old, new in key_map.items():
    content = content.replace('{' + old + '}', '{' + new + '}')
with open('paperorchestra/drafts/intro_relwork.tex', 'w') as f:
    f.write(content)
```
...
python skills/paper-orchestra/scripts/validate_inputs.py --paperorchestra /tmp/po-test/
```

...
- `paperorchestra/outline.json` — specifically the `plotting_plan` array
- `paperorchestra/inputs/idea.md` and `paperorchestra/inputs/experimental_log.md` —
  the source data
- `paperorchestra/inputs/figures/` — optional pre-existing figures (`PlotOn` mode)

## Outputs

- `paperorchestra/figures/<figure_id>.png` — one PNG per `plotting_plan` entry
  (300 DPI, sized to the requested aspect ratio)
- `paperorchestra/figures/captions.json` — `{figure_id: caption_text}` map

## Workflow

### 0.5. Inject Scientific Plotting Standard
CRITICAL: Before generating any plots, you MUST explicitly read and adhere to the **`standards/scientific_plotting_standard.md`** file bundled with this skill. This ensures all generated figures meet the exact DPI, color palette, and visual hierarchy rules required for publication.
...
4. **Render**:
   ...
   **If `PAPERBANANA_PATH` is set** — use the PaperBanana backbone
   ...
   python skills/plotting-agent/scripts/paperbanana_render.py \
       ...
       --content-file paperorchestra/inputs/idea.md \
       --task      <diagram|plot> \
       --aspect-ratio <aspect_ratio> \
       --out       paperorchestra/figures/<figure_id>.png
   ```
...
6. **Generate the caption** using the verbatim Caption Generation prompt at
   ...
   Write the caption to `paperorchestra/figures/captions.json` keyed by
...
If `paperorchestra/inputs/figures/` is non-empty, check whether any pre-existing
file matches a `figure_id` in the outline (by filename prefix). If so,
**copy** it into `paperorchestra/figures/` as-is and **still generate a caption**
